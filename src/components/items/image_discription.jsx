...
function Image({ src, desc, setShowImage }) {
    return (
        <div className={`fixed inset-0 z-10  bg-white/30`} onClick={() => { setShowImage(false) }}>
            ...
            <div class="relative border-3 mt-24 border-black shadow-[12px_12px_0px_#000000] overflow-hidden transition-all ease-in-out duration-300 transform -translate-x-6 -translate-y-6 bg-blue-900 w-fit h-fit p-3 self-center justify-self-center rounded-2xl text-right">
                <div >
                ...
                <div class="content ">
                    <img src={src} className='h-95 w-85 rounded-2xl' />
                    <div className='text-lg text-wrap w-80 text-center p-1 mt-1'>Medium Used: {desc}</div>
                </div>
            </div>
        </div>
    );
